...
func (r *PenaltyRepository) Create(ctx context.Context, q ports.Querier, penalty *domain.Penalty) error {
	...
	if err != nil {
		slog.Error("penalty_repository.Create.exec_query", "error", err, "employee_uid", penalty.EmployeeUID)
		return err
	}

	id, err := result.LastInsertId()
	if err != nil {
		slog.Error("penalty_repository.Create.last_insert_id", "error", err, "employee_uid", penalty.EmployeeUID)
		return err
	}
	penalty.ID = id
...
func (r *PenaltyRepository) IsRemoved(ctx context.Context, q ports.Querier, penaltyID int64) (bool, error) {
	query := `SELECT EXISTS(SELECT 1 FROM penalties_removed WHERE penalty_id = ?)`
...
func (r *PenaltyRepository) CreateRemoval(ctx context.Context, q ports.Querier, removal *domain.PenaltyRemoval) error {
	...
	if err != nil {
		slog.Error("penalty_repository.CreateRemoval.exec_query", "error", err, "penalty_id", removal.PenaltyID)
		return err
	}

	id, err := result.LastInsertId()
	if err != nil {
		slog.Error("penalty_repository.CreateRemoval.last_insert_id", "error", err, "penalty_id", removal.PenaltyID)
		return err
	}
	removal.ID = id
...
func (r *PenaltyRepository) DeleteByEmployeeUID(ctx context.Context, q ports.Querier, employeeUID string) error {
	query := `DELETE FROM penalties WHERE employee_uid = ?`
